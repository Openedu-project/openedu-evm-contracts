...
contract SponsorNft is ERC721, Ownable2Step, EIP712 {
    ...
    mapping(uint256 tokenId => bool isMinted) private s_isMinted;
    ...
    function mintWithPermit(
        ...
        if (s_isMinted[tokenId]) {
            revert SponsorNft__AlreadyMinted();
        }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (ownerOf(tokenId) == address(0)) {
            revert SponsorNft__TokenNotFound();
        }
